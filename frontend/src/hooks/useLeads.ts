...
export function useLeads(params: LeadsQueryParams = {}) {
  const { page = 1, page_size = 10, search = '', status = 'all', sub_source_id, campaign_status = 'all' } = params

  return useQuery({
    queryKey: ['leads', { page, page_size, search, status, sub_source_id, campaign_status }],
